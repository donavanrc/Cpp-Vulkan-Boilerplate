...
def version_is_newer(current, installed):
    if not installed:
        return True
    if current == "master" or installed == "master":
    ...
    try:
        ...
        for c, i in zip(current_parts, installed_parts):
            c_num = int(''.join(filter(str.isdigit, c or '0')))
            i_num = int(''.join(filter(str.isdigit, i or '0')))
            if c_num > i_num:
                return True
            elif c_num < i_num:
                return False
...
def clone_or_update_repo(dep, external_dir):
    ...
    print(f"[BOOTSTRAP] {name} checked out at revision {revision} (commit hash: {commit_hash})")
    return revision
        ...
        with urllib.request.urlopen(request) as response:
            ...
            with open(target_path, 'wb') as out_file:
                ...
                while True:
                    ...
                    if total_size > 0:
                        ...
                        if percent != last_percent:
                            ...
                            if percent == 100:
                                sys.stdout.write(f"[BOOTSTRAP] Download complete: {format_size(downloaded_size)}")
                            else:
                                sys.stdout.write(f"[BOOTSTRAP] {bar} {percent}% ({formatted_downloaded}/{formatted_total})")
                            sys.stdout.flush()
                ...
                if progress_shown:
                    ...
                    print(f"[BOOTSTRAP] Download complete: {format_size(downloaded_size)}")
        return target_path
...
def process_archive(dep, archives_dir, source_dir, tracked_versions):
    ...
    if is_zip and downloaded_path.suffix == '.zip':
        try:
            ...
            return False
    tracked_versions[name] = {'type': 'asset', 'url': url, 'version': os.path.getsize(downloaded_path)}
    return True
        ...
        try:
            if dep_type == 'git':
                ...
                if version_is_newer(current_version, installed_version):
                    revision = clone_or_update_repo(dep, EXTERNAL_DIR)
                    tracked_versions[name] = {'type': 'git', 'url': dep['url'], 'version': revision}
                    save_tracked_versions(tracked_versions, DEPENDENCIES_STATUS_FILE)
                    print(f"[BOOTSTRAP] Successfully installed/updated {name} to version {revision}")
                    success_count += 1
                else:
